...
const kauthUrl = `//kauth.kakao.com/oauth/authorize?client_id=2d1c91f12a4c8020dbcc39ddb0c368b0&redirect_uri=http://${process.env.REACT_APP_SERVER_IP}:3000/kakao/signin&response_type=code`;
...
function SignIn() {
  ...
  async function singInUser(data) {
    await fetch(
      `http://${process.env.REACT_APP_SERVER_IP}/users/signin`,
      {
        method: "post",
        headers: {
